...
_parser.add_argument('--task', default='Seg_Dep_Nor', type=str, 
                        help='choose which task to train Seg for segmentation, Dep for depth, Nor for normal and combined tasks with _ between and Seg_Dep_Nor for all tasks')
...
def prepare_args(params):
    ...
    if params.arch in ['HPS', 'Cross_stitch', 'MTAN', 'CGC', 'PLE', 'MMoE', 'DSelect_k', 'DIY', 'LTB', 'TIT']:
        ...
        if params.arch in ['TIT']:
            kwargs['arch_args']['task'] = params.task
