...
class Trainer(nn.Module):
    ...
    def __init__(self, task_dict, weighting, architecture, encoder_class, decoders, 
        ...
        self.colorjitter = T.transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2)
        self.scale = T.transforms.Resize(size=(288, 384))
    ...
    def train_sl(self, labeled_dataloader, test_dataloaders, epochs, 
              val_dataloaders=None, return_weight=False,unlabeled_dataloader=None, bs=None, labeled_dataset=None, unlabeled_dataset=None, ulw = 1.0):
        r'''The training process of multi-task learning.
        ...
        for epoch in range(epochs):
            ...
            self.model.train()
            self.meter.record_time('begin')
            if epoch <= epp : #itmeans n+1 epochs
                for batch_index in range(train_batch):
                    if not self.multi_input:
                        train_inputs, train_gts = self._process_data(train_loader)
                        train_preds = self.model(train_inputs)
                ...
                with torch.no_grad():
                    for batch_index in range(u_train_batch):
                        ...
                        # Horizontal flip
                        u_train_inputs_hflip = torch.flip(u_train_inputs, [3])
                        u_train_preds_hflip = self.model(u_train_inputs_hflip)
                        u_train_preds_hflip = self.process_preds(u_train_preds_hflip)
                        del u_train_inputs_hflip
                        # Color jitter
                        u_train_inputs_color = self.colorjitter(u_train_inputs)
                        u_train_preds_color = self.model(u_train_inputs_color)
                        u_train_preds_color = self.process_preds(u_train_preds_color)
                        del u_train_inputs_color
                        # Scaling
                        u_train_inputs_scaled = self.scale(u_train_inputs)
                        u_train_preds_scaled = self.model(u_train_inputs_scaled)
                        u_train_preds_scaled = self.process_preds(u_train_preds_scaled)
                        del u_train_inputs_scaled
                        for key, value in u_train_preds.items():
                            if key == 'segmentation':
                                u_train_preds[key] = F.softmax(value, dim=1)
                                u_train_preds_hflip[key] = F.softmax(u_train_preds_hflip[key], dim=1)
                                u_train_preds_color[key] = F.softmax(u_train_preds_color[key], dim=1)
                                u_train_preds_scaled[key] = F.softmax(u_train_preds_scaled[key], dim=1)
                        for key in u_train_preds_scaled:
                            u_train_preds_hflip[key] = torch.flip(u_train_preds_hflip[key], [3])
                            u_train_preds_scaled[key] = F.interpolate(u_train_preds_scaled[key], size=u_train_inputs.shape[2:4], mode='bilinear', align_corners=False)
                        for key in u_train_preds:
                            u_train_preds[key] = (u_train_preds[key] + u_train_preds_hflip[key] +
                                                u_train_preds_color[key] + 
                                                u_train_preds_scaled[key]) / 4.0
                        for key, value in u_train_preds.items():
                            if key not in pseudo_labels:
                                pseudo_labels[key] = []
                            pseudo_labels[key].append(value.cpu())
    # Concatenate pseudo labels for all batches for each modality
                for key, value in pseudo_labels.items():
                    ...
                    pseudo_labels[key] = pseudo_labels[key].to('cpu')
                ...
                for batch_index in range(c_train_batch):
                    ...
                    c_train_inputs_hflip = torch.flip(c_train_inputs, [3])
                    c_train_preds_hflip = self.model(c_train_inputs_hflip)
                    c_train_preds = self.process_preds(c_train_preds_hflip)
                    for key, value in c_train_gts.items():
                        c_train_gts[key] = torch.flip(c_train_gts[key], [3])
                    c_train_preds = self.process_preds(c_train_preds)
                    # train_losses = self._compute_loss(c_train_preds, c_train_gts)
                    c_train_preds_l = {k:None for k in c_train_preds.keys()}
                    c_train_preds_u = {k:None for k in c_train_preds.keys()}
                    c_train_gts_l = {k:None for k in c_train_preds.keys()}
                    c_train_gts_u = {k:None for k in c_train_preds.keys()}
                    for key, value in c_train_preds.items():
                        c_train_preds_l[key], c_train_preds_u[key] = c_train_preds[key][:int(bs/2)], c_train_preds[key][int(bs/2):] #number of labeled
                        c_train_gts_l[key], c_train_gts_u[key] = c_train_gts[key][:int(bs/2)], c_train_gts[key][int(bs/2):]
                    train_losses_l = self._compute_loss(c_train_preds_l,c_train_gts_l)
                    train_losses_u = self._compute_loss(c_train_preds_u,c_train_gts_u)
                    new_train_loss = train_losses_l + (ulw * train_losses_u)
                    self.meter.update(c_train_preds, c_train_gts)
                    self.optimizer.zero_grad(set_to_none=False)
                    w = self.model.backward(new_train_loss, **self.kwargs['weight_args'])
                    # if w is not None:
                    #     self.batch_weight[:, epoch, batch_index] = w
                    self.optimizer.step()
                # del combined_loader
